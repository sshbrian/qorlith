...
describe('director video plan', () => {
  ...
  it('Straight to video director prompt is T2VA, not a start still', () => {
    ...
    assert.match(cont, /The previous last frame IS frame 0/)
    assert.doesNotMatch(cont, /The start still IS frame 0/)
    assert.match(cont, /camera \+ body action only/)
    const stillsCont = buildVideoSystemPrompt({ continueFromPrior: true })
    assert.match(stillsCont, /The previous last frame IS frame 0/)
    assert.doesNotMatch(stillsCont, /The start still IS frame 0/)
  })
})
