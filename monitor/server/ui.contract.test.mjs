...
describe('UI contract — stages, not tools', () => {
  ...
  it('Home and Plan expose Straight to video', () => {
    ...
    assert.match(page('StudioHome.tsx'), /The lights are coming up/)
    assert.match(page('Watch.tsx'), /The house is dark/)
    assert.doesNotMatch(read(path.join(src, 'components', 'Layout.tsx')), /No projects yet/)
    ...
    assert.match(page('StudioHome.tsx'), /tonightFilm/)
    assert.match(page('StudioHome.tsx'), /filmsJustCanned/)
    assert.match(page('StudioHome.tsx'), /is-canned/)
    assert.match(page('StudioHome.tsx'), /writeTonightId/)
    assert.match(page('StudioHome.tsx'), /readTonightId/)
    assert.match(page('StudioHome.tsx'), /toggleHouseMute/)
    assert.match(read(path.join(src, 'components', 'Layout.tsx')), /toggleHouseMute/)
    assert.match(read(path.join(src, 'components', 'Layout.tsx')), /house-word/)
    assert.match(read(path.join(src, 'lib', 'studio.ts')), /TONIGHT_KEY/)
    assert.match(read(path.join(src, 'lib', 'houseSound.ts')), /toggleHouseMute/)
    assert.match(page('StudioHome.tsx'), /Watch again/)
    assert.match(page('StudioHome.tsx'), /lobby-night/)
    assert.match(page('StudioHome.tsx'), /overlay/)
    assert.match(read(path.join(src, 'index.css')), /has-night \.poster-wall-grid/)
    assert.match(read(path.join(src, 'index.css')), /table-print\.is-pinned::after/)
    assert.match(page('EpisodePlan.tsx'), /table-hero.*is-pinned/)
    assert.match(page('EpisodePlan.tsx'), /readBoardScene/)
    assert.match(page('EpisodePlan.tsx'), /writeBoardScene/)
    assert.match(page('EpisodePlan.tsx'), /studioCoverHang/)
    assert.match(read(path.join(src, 'lib', 'api.ts')), /studioCoverHang/)
    assert.match(read(path.join(ROOT, 'server', 'project.mjs')), /setProjectCover/)
    assert.match(read(path.join(src, 'index.css')), /chalk-land/)
    assert.match(page('StudioPlanner.tsx'), /VideoModeToggle/)
    assert.match(page('StudioPlanner.tsx'), /script-sheet/)
  ...
  it('Make hides stills-first chrome in Straight to video', () => {
    ...
    assert.match(t, /printPoster/)
    assert.match(t, /is-landed/)
    assert.match(t, /is-ready/)
    assert.match(t, /requestAnimationFrame/)
    assert.match(t, /scrollTo/)
    assert.match(t, /setTimeout/)
  ...
  it('Home posters can use a video cover', () => {
    ...
    assert.match(read(path.join(src, 'components', 'Layout.tsx')), /houseWhoosh/)
    assert.match(page('EpisodePlan.tsx'), /housePin/)
    assert.match(read(path.join(src, 'lib', 'houseSound.ts')), /prefers-reduced-motion/)
    assert.match(read(path.join(src, 'lib', 'houseSound.ts')), /qorlith.house.mute/)
    assert.match(read(path.join(src, 'index.css')), /view-transition-name: title-card/)
...
describe('UI contract — brain is shipped', () => {
  it('Brain is a LangGraph control room, not a sketch', () => {
    ...
    const brainReadme = read(path.join(ROOT, '..', 'brain', 'README.md'))
    assert.doesNotMatch(brainReadme, /finish concats the clips/)
    assert.match(brainReadme, /finish joins them/)
  ...
  it('Watch is a theater, not a clip dump', () => {
    ...
    assert.match(t, /workprint/)
    assert.match(t, /WorkprintFrame/)
    assert.doesNotMatch(t, /scene-grid/)
    assert.doesNotMatch(t, /controls/)
    assert.match(t, /clips\.filter\(\(c\) => c\.id\)/)
    assert.doesNotMatch(t, /setInterval/)
    assert.match(t, /theater-first/)
    assert.match(t, /theater-credits/)
    assert.match(t, /Made with Qorlith/)
    assert.match(t, /watchFirstFrame/)
    assert.match(t, /clipIndexAtTime/)
    assert.match(t, /houseLights/)
    assert.match(t, /is-picture/)
    assert.match(t, /scrollTo/)
    assert.match(t, /workprint-hair/)
    assert.match(t, /timeAtReel/)
    assert.match(t, /holdAt/)
    assert.match(t, /writeTonightId/)
    assert.match(t, /hangPrint/)
    assert.match(t, /ArrowRight/)
    assert.match(t, /Escape/)
    assert.match(t, /houseWhoosh/)
    assert.match(t, /studioCoverHang/)
    ...
    assert.match(page('Brain.tsx'), /aria-current=\{live \? 'true'/)
